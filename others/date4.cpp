...
class date{
    public:
        ...
        void set_month(int);
        void set_day(int);
        void set_all_three(int, int, int);
    private:
        ...
        int get_day(int, int, int);  //accept a year, day and month, error check day, return
        bool isLeapYear(int);        //check if it is a leap year 
...
int date::get_day(int y, int d, int mon){
    int * d_per_mon = days_per_month;
    if(isLeapYear(y)) 
        d_per_mon = days_per_month_leap_year;
    if(d < 1 || d > d_per_mon[mon-1]){
        int tempd = d;
        while(tempd < 1 || tempd > d_per_mon[mon-1]){
...
void date::set_all_three(int y, int mon, int d){
    year = get_year(y);
    month = get_month(mon);
    day = get_day(year, d, month);
}
